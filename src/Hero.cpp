...
void Hero::MakeHeroes() {
	...
	// Read data from heroes.csv. Take all the information together and make the Hero objects.
	file.open(DATA_DIR + "/heroes.csv");
	while (file.good()) {
		std::string line{};
		std::getline(file, line);
		if (line == "") { // empty lines for grouping, just skip
	...
	if(Menu::validating) {
		// validate hero names in owned.csv
...
void Hero::ValidateHeroNames(const std::vector<std::string>& hero_names, std::string check_name) {
	for(std::string hero_name: hero_names) {
		...
		if(!hero_found) {
			std::string error_message = "Hero name " + hero_name + " in " + check_name + " was not found in data/heroes.csv.";
	...
	pvp_tier2{ std::stoi(data[9]) }, pve_tier2{ std::stoi(data[10]) }, owned{ false }, acquisition{}, upgrades{}
{
	...
	// this->upgrades is the vector with the hero's upgrades
