...
void TopLevelMenu() {
  ...
  size_t int_input = AskForInput(
      "You can choose from the following capabilities.", function_names);
  std::cout << std::endl;
...
int GetIntInput(int min, int max) {
  ...
  try {
    ...
    std::string error_message = "You must only enter a number between " +
                                std::to_string(min) + " and " +
                                std::to_string(max) + ".";
    throw std::runtime_error(error_message);
  } catch (const std::invalid_argument &) { // std::stoi couldn't convert
    std::string error_message = "You must enter a number between " +
                                std::to_string(min) + " and " +
                                std::to_string(max) + ".";
    throw std::runtime_error(error_message);
  }
...
size_t AskForInput(const std::string &prompt,
                   const std::vector<std::string> &options, bool min_is_zero) {
  if (options.empty()) {
  ...
  return static_cast<size_t>(
      GetIntInput(static_cast<int>(min), static_cast<int>(max)));
}
