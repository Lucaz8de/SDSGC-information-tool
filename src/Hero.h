...
struct Hero {
	...
	/** @brief The hero's rank on some different tierlists (a number between 1 and 5) */
	...
	/** @brief A list of the ways the hero can currently be obtained in-game. Empty for unobtainable heroes. */
	 ...
	 */
	Hero(const std::array<std::string, INFORMATION_COUNT>& data, std::deque<std::pair<std::string, std::array<std::string, Hero::UPGRADES_COUNT>>> &upgrades, const std::unordered_map<std::string, std::vector<std::string>>& acquisition);
